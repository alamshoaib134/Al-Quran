...
def main():
    ...
    """
    # Load data
    (quran_data, quran_arabic, quran_urdu, quran_hindi,
     quran_bangla, quran_tamil, quran_malayalam, surah_names) = load_data()
    ...
    with gr.Blocks(css=load_css()) as iface:
        ...
        - Read complete Surahs in multiple languages (Arabic, English, Urdu, Hindi, Bengali, Tamil, Malayalam) with word cloud visualizations
        """)
        
        # Create tabs
        search_tab = create_search_tab(quran_data)
        statistics_tab = create_statistics_tab(quran_data)
        surah_tab = create_surah_tab(quran_data, quran_arabic, quran_urdu, quran_hindi,
                                    quran_bangla, quran_tamil, quran_malayalam, surah_names_list)
